...
<div class="btn-list">
  ...
  @can('language.create')
  <a href="{{ route('admin.system.languages.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <p class="card-subtitle">Manage system languages and translations</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="languages-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#languages-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.languages.index") }}',
        columns: [
            ...
            { data: 'native_name', name: 'native_name', searchable: true },
            { data: 'code', name: 'code', searchable: true },
            { data: 'direction', name: 'direction', searchable: true },
            { data: 'default_badge', name: 'default', orderable: false, searchable: false },
        ...
        dom: 'Brt',
        buttons: [
            { extend: 'excel', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6] } },
            { extend: 'pdf', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6] } },
            { extend: 'csv', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6] } },
            { extend: 'copy', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6] } },
            { extend: 'print', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6] } }
        ],
    ...
    $('#import-btn').on('click', function() {
        showToast('Info', 'Import functionality coming soon', 'info');
    });
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete language',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `You are about to delete ${selectedIds.length} language(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} language(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.languages.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
