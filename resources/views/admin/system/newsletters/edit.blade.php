...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <p class="card-subtitle">Update your newsletter campaign</p>
    </div>
  </div>
  <form action="{{ route('admin.system.newsletters.update', $newsletter->id) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="card-body">
      <div class="mb-3">
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control @error('body_html') is-invalid @enderror" 
                  id="body_html" name="body_html" rows="15">{{ old('body_html', $newsletter->body_html) }}</textarea>
        @error('body_html')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
      </div>
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control @error('body_text') is-invalid @enderror" 
                  id="body_text" name="body_text" rows="10">{{ old('body_text', $newsletter->body_text ?? strip_tags($newsletter->body_html)) }}</textarea>
      ...
      <div class="row">
        <div class="col-md-6">
          <div class="mb-3">
            ...
            <select class="form-select @error('status') is-invalid @enderror" 
              ...
              <option value="sending" {{ old('status', $newsletter->status) === 'sending' ? 'selected' : '' }}>Sending</option>
              <option value="sent" {{ old('status', $newsletter->status) === 'sent' ? 'selected' : '' }}>Sent</option>
              <option value="cancelled" {{ old('status', $newsletter->status) === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
            </select>
            @error('status')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        ...
        <div class="col-md-6" id="schedule-section" style="display: {{ in_array(old('status', $newsletter->status), ['scheduled', 'sending']) ? 'block' : 'none' }};">
          <div class="mb-3">
            ...
            <input type="datetime-local" class="form-control @error('scheduled_for') is-invalid @enderror" 
                   ...
                   value="{{ old('scheduled_for', $newsletter->scheduled_for ? $newsletter->scheduled_for->format('Y-m-d\\TH:i') : '') }}">
            @error('scheduled_for')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
            <small class="form-hint">Set the date and time when this newsletter should be sent</small>
          </div>
        </div>
      </div>
    </div>
...
$(document).ready(function() {
  ...
  $('#status').on('change', function() {
    if ($(this).val() === 'scheduled' || $(this).val() === 'sending') {
