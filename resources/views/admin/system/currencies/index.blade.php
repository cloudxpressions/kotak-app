...
<div class="btn-list">
  ...
  @can('currency.create')
  <a href="{{ route('admin.system.currencies.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <p class="card-subtitle">Manage currencies and exchange rates for your application</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="currencies-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#currencies-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.currencies.index") }}',
        columns: [
            ...
            { data: 'symbol', name: 'symbol', searchable: true },
            { data: 'conversion_rate', name: 'conversion_rate', searchable: true },
            { data: 'symbol_position', name: 'symbol_position', searchable: true },
            { data: 'decimal_places', name: 'decimal_places', searchable: true },
        ...
        dom: 'Brt',
        buttons: [
            { extend: 'excel', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6, 7, 8] } },
            { extend: 'pdf', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6, 7, 8] } },
            { extend: 'csv', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6, 7, 8] } },
            { extend: 'copy', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6, 7, 8] } },
            { extend: 'print', className: 'd-none', exportOptions: { columns: [1, 2, 3, 4, 5, 6, 7, 8] } }
        ],
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/system/currencies/${id}`,
                    method: 'DELETE',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete currency',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `You are about to delete ${selectedIds.length} currency(ies). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} currency(ies)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.currencies.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
