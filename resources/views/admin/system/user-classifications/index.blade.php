...
@section('page-title', 'User Classifications')
...
<div class="btn-list">
  ...
  @can('userclassification.create')
  <a href="{{ route('admin.system.user-classifications.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">User Classification Management</h3>
      <p class="card-subtitle">Manage user classification types for your system</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="user-classifications-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#user-classifications-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.user-classifications.index") }}',
        columns: [
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/system/user-classifications/${id}`,
                    method: 'DELETE',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete user classification',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            title: 'Delete Multiple User Classifications?',
            text: `You are about to delete ${selectedIds.length} user classification(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} user classification(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.user-classifications.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
