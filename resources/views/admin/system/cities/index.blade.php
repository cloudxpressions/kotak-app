...
<div class="btn-list">
  ...
  @can('city.update')
  <a href="{{ route('admin.system.cities.activate') }}" class="btn btn-outline-success">
  ...
  @can('city.create')
  <a href="{{ route('admin.system.cities.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <p class="card-subtitle">Manage system cities and their details</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="cities-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#cities-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.cities.index") }}',
        columns: [
            ...
            { data: 'state_name', name: 'state.name', searchable: true },
            { data: 'country_name', name: 'country.name', searchable: true },
            { data: 'is_district_hq', name: 'is_district_hq', searchable: false, render: function(data) {
                return data ? '<span class="badge bg-success-lt">Yes</span>' : '<span class="badge bg-secondary-lt">No</span>';
            }},
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete city',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `You are about to delete ${selectedIds.length} city(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} city(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.cities.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
