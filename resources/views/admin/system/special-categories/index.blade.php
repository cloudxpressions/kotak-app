...
@section('page-title', 'Special Categories')
...
<div class="btn-list">
  ...
  <div class="btn-group">
    ...
    <ul class="dropdown-menu">
      ...
      <li><a class="dropdown-item" href="#" id="export-print">
        <svg xmlns="http://www.w3.org/2000/svg" class="icon me-2" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M17 17h2a2 2 0 0 0 2 -2v-4a2 2 0 0 0 -2 -2h-14a2 2 0 0 0 -2 2v4a2 2 0 0 0 2 2z" /><path d="M17 9v-4a2 2 0 0 0 -2 -2h-6a2 2 0 0 0 -2 2v4" /><path d="M7 13m0 2a2 2 0 0 1 2 -2h6a2 2 0 0 1 2 2v4a2 2 0 0 1 -2 2h-6a2 2 0 0 1 -2 -2z" /></svg>
        Print
      </a></li>
    </ul>
  </div>

  @can('specialcategory.delete')
  ...
  @can('specialcategory.create')
  <a href="{{ route('admin.system.special-categories.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">Special Category Management</h3>
      <p class="card-subtitle">Manage special categories for your system</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="special-categories-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#special-categories-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.special-categories.index") }}',
        columns: [
        ...
        dom: 'Brt',
        buttons: [
            { extend: 'excel', className: 'd-none', exportOptions: { columns: [1, 2] } },
            { extend: 'pdf', className: 'd-none', exportOptions: { columns: [1, 2] } },
            { extend: 'csv', className: 'd-none', exportOptions: { columns: [1, 2] } },
            { extend: 'copy', className: 'd-none', exportOptions: { columns: [1, 2] } },
            { extend: 'print', className: 'd-none', exportOptions: { columns: [1, 2] } }
        ],
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/system/special-categories/${id}`,
                    method: 'DELETE',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete special category',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            title: 'Delete Multiple Special Categories?',
            text: `You are about to delete ${selectedIds.length} special category(ies). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} special category(ies)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.special-categories.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
