...
@section('page-title', 'Time Zones')

@section('breadcrumbs')
<ol class="breadcrumb" aria-label="breadcrumbs">
  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
  <li class="breadcrumb-item"><a href="#">System</a></li>
  <li class="breadcrumb-item active" aria-current="page">Time Zones</li>
</ol>
@endsection
...
<div class="btn-list">
  ...
  @can('timezone.create')
  <a href="{{ route('admin.system.time-zones.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">Time Zone Management</h3>
      <p class="card-subtitle">Manage time zones for your system</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="time-zones-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#time-zones-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.time-zones.index") }}',
        columns: [
            ...
            { data: 'timezone', name: 'timezone', searchable: true },
            { data: 'offset', name: 'offset', searchable: true },
            { data: 'utc_offset_minutes', name: 'utc_offset_minutes', searchable: false },
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/system/time-zones/${id}`,
                    method: 'DELETE',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete time zone',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            title: 'Delete Multiple Time Zones?',
            text: `You are about to delete ${selectedIds.length} time zone(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} time zone(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.time-zones.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
