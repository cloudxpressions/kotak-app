...
<div class="btn-list">
  ...
  @can('state.update')
  <a href="{{ route('admin.system.states.activate') }}" class="btn btn-outline-success">
  ...
  @can('state.create')
  <a href="{{ route('admin.system.states.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      ...
      <p class="card-subtitle">Manage system states and their details</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="states-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#states-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.system.states.index") }}',
        columns: [
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete state',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `You are about to delete ${selectedIds.length} state(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} state(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.system.states.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
