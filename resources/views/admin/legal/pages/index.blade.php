...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">Legal Pages Management</h3>
      <p class="card-subtitle">Manage your legal pages like Terms, Privacy Policy, etc.</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="legal-pages-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#legal-pages-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.legal.pages.index") }}',
        columns: [
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete legal page',
                            icon: 'error'
                        });
                    }
                });
...
$('#bulk-delete-btn').on('click', function() {
    ...
    Swal.fire({
        title: 'Delete Multiple Legal Pages?',
        text: `You are about to delete ${selectedIds.length} legal page(s). This action cannot be undone!`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#d63939',
        cancelButtonColor: '#6c757d',
        confirmButtonText: `Yes, delete ${selectedIds.length} legal page(s)!`,
        cancelButtonText: 'Cancel'
    }).then((result) => {
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("admin.legal.pages.bulk-delete") }}',
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    ids: selectedIds
