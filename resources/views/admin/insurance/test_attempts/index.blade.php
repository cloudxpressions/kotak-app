...
<ol class="breadcrumb" aria-label="breadcrumbs">
  ...
  <li class="breadcrumb-item active" aria-current="page">Test Attempts</li>
</ol>
@endsection

@section('page-actions')
<div class="btn-list">
  @can('test_attempt.delete')
...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">Test Attempts Management</h3>
      <p class="card-subtitle">Monitor user test attempts and scores</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="test-attempts-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#test-attempts-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.insurance.test_attempts.index") }}',
        columns: [
            ...
            { data: 'test', name: 'test', orderable: true, searchable: true },
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/insurance/test-attempts/${id}`,
                    method: 'DELETE',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete test attempt',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            title: 'Delete Multiple Test Attempts?',
            text: `You are about to delete ${selectedIds.length} test attempt(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} test attempt(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.insurance.test_attempts.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
