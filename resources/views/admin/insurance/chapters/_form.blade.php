        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('exam_id') is-invalid @enderror" name="exam_id" required>
                                ...
                                <option value="{{ $exam->id }}" {{ old('exam_id', $chapter->exam_id ?? '') == $exam->id ? 'selected' : '' }}>
                                    {{ $exam->code }} - {{ $exam->name }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('insurance_category_id') is-invalid @enderror" name="insurance_category_id" required>
                                <option value="">Select Category</option>
                                @foreach($categories as $category)
                                <option value="{{ $category->id }}" {{ old('insurance_category_id', $chapter->insurance_category_id ?? '') == $category->id ? 'selected' : '' }}>
                                    {{ $category->slug }} - {{ $category->name }}
                                </option>
                                @endforeach
                            </select>
                            @error('insurance_category_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('order_no') is-invalid @enderror" name="order_no" value="{{ old('order_no', $chapter->order_no ?? '') }}">
                    @error('order_no')
                    <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <label class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $chapter->is_active ?? true) ? 'checked' : '' }}>
    ...
    <div class="card-body">
        <div id="translations-container">
            @if(old('translations'))
                ...
                @endforeach
            @elseif(isset($chapter) && $chapter->translations->count() > 0)
                @foreach($chapter->translations as $index => $translation)
                <div class="translation-row mb-3 p-3 border rounded">
                    <div class="d-flex justify-content-between align-items-start mb-2">
...
document.addEventListener('DOMContentLoaded', function() {
    let translationIndex = {{ old('translations') ? count(old('translations')) : (isset($chapter) ? $chapter->translations->count() : 0) }};
