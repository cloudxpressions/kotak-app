...
@section('page-title', 'User Saved Items')

@section('breadcrumbs')
<ol class="breadcrumb" aria-label="breadcrumbs">
  <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
  <li class="breadcrumb-item"><a href="#">Insurance</a></li>
  <li class="breadcrumb-item active" aria-current="page">User Saved Items</li>
</ol>
@endsection

@section('page-actions')
<div class="btn-list">
  @can('user_saved_item.delete')
...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">User Saved Items Management</h3>
      <p class="card-subtitle">Manage user bookmarks and pins</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="user-saved-items-table">
      <thead>
        <tr>
...
$(document).ready(function() {
    ...
    const table = $('#user-saved-items-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.insurance.user_saved_items.index") }}',
        columns: [
            ...
            { data: 'entity_type_badge', name: 'entity_type', orderable: true, searchable: true },
            { data: 'entity_id', name: 'entity_id', orderable: true, searchable: true },
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/insurance/user-saved-items/${id}`,
                    method: 'DELETE',
                    ...
                    error: function(xhr) {
                        Swal.fire({
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to delete user saved item',
                            icon: 'error'
                        });
                    }
                });
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            title: 'Delete Multiple User Saved Items?',
            text: `You are about to delete ${selectedIds.length} user saved item(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} user saved item(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.insurance.user_saved_items.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
