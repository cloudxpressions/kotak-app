    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">One Liner Information</h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('chapter_id') is-invalid @enderror" name="chapter_id" required>
                                ...
                                <option value="{{ $chapter->id }}" {{ old('chapter_id', $oneLiner->chapter_id ?? '') == $chapter->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('order_no') is-invalid @enderror" name="order_no" value="{{ old('order_no', $oneLiner->order_no ?? '') }}">
                            @error('order_no')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="mb-3">
                    ...
                    <label class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="is_active" value="1" {{ old('is_active', $oneLiner->is_active ?? true) ? 'checked' : '' }}>
    ...
    <div class="card-body">
        <div id="translations-container">
            @if(old('translations'))
                ...
                @endforeach
            @elseif(isset($oneLiner) && $oneLiner->translations->count() > 0)
                @foreach($oneLiner->translations as $index => $translation)
                <div class="translation-row mb-3 p-3 border rounded">
                    <div class="d-flex justify-content-between align-items-start mb-2">
...
document.addEventListener('DOMContentLoaded', function() {
    let translationIndex = {{ old('translations') ? count(old('translations')) : (isset($oneLiner) ? $oneLiner->translations->count() : 0) }};
