...
@section('page-title', 'Insurance Categories')
...
<div class="btn-list">
  ...
  @can('insurance_category.create')
  <a href="{{ route('admin.insurance.categories.create') }}" class="btn btn-primary">
    <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
      <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
      <path d="M12 5l0 14" />
      <path d="M5 12l14 0" />
...
<div class="card">
  <div class="card-header">
    <div>
      <h3 class="card-title">Insurance Category Management</h3>
      <p class="card-subtitle">Manage insurance categories and their translations</p>
    </div>
  </div>
  ...
  <div class="table-responsive">
    <table class="table card-table table-vcenter text-nowrap datatable" id="categories-table">
      <thead>
        <tr>
          <th class="w-1"><input type="checkbox" id="select-all" class="form-check-input"></th>
          <th>Name (All Languages)</th>
          <th>Status</th>
...
$(document).ready(function() {
    ...
    const table = $('#categories-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.insurance.categories.index") }}',
        columns: [
    ...
    $(document).on('click', '.delete-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/insurance/categories/${id}`,
                    method: 'DELETE',
    ...
    $('#bulk-delete-btn').on('click', function() {
        ...
        Swal.fire({
            ...
            text: `You are about to delete ${selectedIds.length} category(s). This action cannot be undone!`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#d63939',
            cancelButtonColor: '#6c757d',
            confirmButtonText: `Yes, delete ${selectedIds.length} category(s)!`,
            cancelButtonText: 'Cancel'
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: '{{ route("admin.insurance.categories.bulk-delete") }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
