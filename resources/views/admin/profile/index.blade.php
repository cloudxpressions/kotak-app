    ...
    <div class="row g-0">
      ...
      <div class="col-12 col-md-3 border-end">
        <div class="card-body">
          ...
          <nav class="list-group list-group-transparent" id="profileTabNav" role="tablist">
          ...
          <nav class="list-group list-group-transparent">
            <a href="#education-panel" class="list-group-item list-group-item-action d-flex align-items-center panel-link" data-panel-target="#education-panel">Education</a>
            <a href="#skills-panel" class="list-group-item list-group-item-action d-flex align-items-center panel-link" data-panel-target="#skills-panel">Skills</a>
            <a href="#password-panel" class="list-group-item list-group-item-action d-flex align-items-center panel-link" data-panel-target="#password-panel">Change Password</a>
          </nav>
        </div>
      </div>
      ...
      <div class="col-12 col-md-9 d-flex flex-column">
        <div id="profileFormContainer" class="d-flex flex-column flex-grow-1">
          <form action="{{ route('admin.profile.update-basic-info') }}" method="POST" id="profileForm" enctype="multipart/form-data">
            @csrf
            @method('PUT')
...
<div class="tab-pane fade show active" id="tab-personal" role="tabpanel" aria-labelledby="tab-personal-tab">
  ...
  <div class="row g-3 mt-2">
    <div class="col-md-6">
      ...
      <input type="date" class="form-control @error('dob') is-invalid @enderror"
             name="dob"
             value="{{ old('dob', $admin->dob ? $admin->dob->format('Y-m-d') : '') }}">
    ...
    <div class="col-md-6">
      <label class="form-check mt-2">
        <input type="checkbox"
               ...
               value="1"
               {{ old('is_differently_abled', $admin->is_differently_abled) ? 'checked' : '' }}>
    ...
    <div class="col-md-6 d-none" id="da_category_div">
      ...
      <select class="form-select select2" name="d_a_category_id">
        ...
        @foreach($daCategories as $dac)
          <option value="{{ $dac->id }}" {{ old('d_a_category_id', $admin->d_a_category_id) == $dac->id ? 'selected' : '' }}>
            {{ $dac->name }}
          </option>
        @endforeach
      </select>
  ...
  <div class="row g-3 mt-2">
    <div class="col-md-12">
      ...
      <input type="text" class="form-control @error('short_bio') is-invalid @enderror"
             name="short_bio"
             value="{{ old('short_bio', $admin->short_bio) }}">
  ...
  <div class="row g-3 mt-2">
    <div class="col-md-12">
      ...
      @endif
        ...
        {{-- Education Section --}}
        <div id="education-panel" class="card-body border-top additional-panel d-none">
          <h2 class="mb-4">Education</h2>
          @include('admin.profile.partials.education-modals')
        </div>

        {{-- Skills Section --}}
        <div id="skills-panel" class="card-body border-top additional-panel d-none">
          <h2 class="mb-4">Skills</h2>
          @include('admin.profile.partials.skills-modals')
        </div>

        {{-- Password Section --}}
        <div id="password-panel" class="card-body border-top additional-panel d-none">
...
$(document).ready(function() {
  ...
  const formContainer = $('#profileFormContainer');
  const tabLinks = $('#profileTabNav [data-bs-toggle="tab"]');
  const panelLinks = $('.panel-link');
  const additionalPanels = $('.additional-panel');
  ...
  const firstInvalid = $('#profileForm .is-invalid').first();
  ...
  $(document).on('click', '.edit-education-btn', function() {
    const button = $(this);
    $('#edit_qualification').val(button.data('qualification'));
    $('#edit_year').val(button.data('year'));
    $('#edit_medium').val(button.data('medium'));
    $('#editEducationForm').attr('action', button.data('update-url'));
  ...
  $(document).on('click', '.edit-skill-btn', function() {
    const button = $(this);
    $('#edit_skill_name').val(button.data('name'));
    $('#edit_proficiency_level').val(button.data('level'));
    $('#edit_skill_description').val(button.data('description'));
    $('#editSkillForm').attr('action', button.data('update-url'));
