...
class AdminAdditionalDetailsController extends Controller
{
    /**
     * Show the detailed profile of an admin (for editing by another admin)
     */
    public function show($id)
    {
        ...
        $states = $admin->country_id
            ? State::select('id', 'name')->where('country_id', $admin->country_id)->get()
            : collect();

        // Only fetch cities if state is selected, otherwise return empty collection
        $cities = $admin->state_id
            ? City::select('id', 'name')->where('state_id', $admin->state_id)->get()
            : collect();
        ...
        return view('admin.admins.details', compact(
            'admin', 'proficiencyLevels', 'countries', 'states', 'cities',
    ...
    /**
     * Update admin's detailed information
     */
    public function updateDetails(Request $request, $id)
    {
        $admin = Admin::select(['id', 'email', 'image'])->findOrFail($id);
    ...
    {
        // Just verify the admin exists without loading all data
        Admin::select('id')->findOrFail($id);
    ...
    /**
     * Update education for admin
     */
    public function updateEducation(Request $request, $id, $educationId)
    {
        // Verify that the education record belongs to this admin
        $education = AdminEducation::where('admin_id', $id)->findOrFail($educationId);
    ...
    /**
     * Update skill for admin
     */
    public function updateSkill(Request $request, $id, $skillId)
    {
        // Verify that the skill record belongs to this admin
        $skill = AdminSkill::where('admin_id', $id)->findOrFail($skillId);
